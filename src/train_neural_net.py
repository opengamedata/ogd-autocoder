...
def train_neural_net(
    ...
    model.eval()
    with torch.no_grad():
        y_prob_test = nn.functional.softmax(model(x_test_tensor), dim=1).cpu().numpy()
        y_prob_train = nn.functional.softmax(model(x_train_tensor), dim=1).cpu().numpy()
    ...
    for i, w in zip(class_labels, class_weights.cpu().numpy()):
